...
function generatesummary(from, to) {
      ...
      if ( simpleprefs.prefs.summary == "_AUTO_" ) {
        ...
        for ( ; i<words.length; i++ )  {
          ...
          if ( to.slice(0,word.length) == word ) {
             to = to.slice(word.length+1,to.length);
             from = from.slice(word.length+1,from.length); }
          else
             break;
        }
        ...
        while (i--) {
          ...
          if ( to.slice(to.length-word.length,to.length) == word ) {
             to = to.slice(0,to.length-word.length-1);
             from = from.slice(0,from.length-word.length-1); }
          else
             break;
        }
        summary = from + " -> " + to;
      ...
      // use summary to user set in the preferences
      else
...
function getsourcecode(article, from, to) {
  var sourcecoderequest = Request({
    ...
    onComplete: function (response) {
      ...
      var summary = generatesummery(from, to);
